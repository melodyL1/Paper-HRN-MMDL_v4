...
class HRN(nn.Module):
    def __init__(self, dataset):
        ...
        self.weights_param = nn.Parameter(
            torch.ones(self.num_modalities + 1, out_class) / (self.num_modalities + 1))
    ...
    def forward(self, x, weights):
        ...
        # 特殊实验：模态注意力权重（可学习参数）
        # weights_tensor = torch.softmax(self.weights_param, dim=0)

        # 特殊实验：消融实验，取消注释下面一行权重默认为1
        ...
        for i in range(self.num_modalities):
            ...
            # 特殊实验：模态去除实验，i条件大于-1开始去除，例如i == 3为去除第四个模态。
            if i == -1:
                outputs_weighted = outputs_weighted
                print("Remove:", i + 1)
            else:
...
class CNN_1D(nn.Module):
    ...
    """
    def __init__(self, dataset, CNN_1D_in_channel=None):
        super(CNN_1D, self).__init__()
        if CNN_1D_in_channel is None:
            CNN_1D_in_channel = in_channel
        self.dataset = dataset
        self.C2 = nn.Sequential(
            nn.Conv1d(in_channels=CNN_1D_in_channel, out_channels=6, kernel_size=(3,)),
            nn.BatchNorm1d(6),
            nn.ReLU(inplace=True),
...
class CNN_2D(nn.Module):
    ...
    """
    def __init__(self, dataset, CNN_2D_in_channel = None):
        super(CNN_2D, self).__init__()
        if CNN_2D_in_channel is None:
            CNN_2D_in_channel = in_channel
        self.dataset = dataset
        self.FE = nn.Sequential(
            nn.Conv2d(in_channels=CNN_2D_in_channel, out_channels=get_in_planes(dataset), kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False),
...
class CNN_3D(nn.Module):
    ...
    def __init__(self, dataset, CNN_3D_in_channel = None):
        super(CNN_3D, self).__init__()
        if CNN_3D_in_channel is None:
            CNN_3D_in_channel = in_channel
        self.dataset = dataset
        out_channels = [32, 64, 128]
        if CNN_3D_in_channel == 1:
            self.layer1 = nn.Sequential(
        ...
        if CNN_3D_in_channel > 1:
            self.classifier = nn.Linear(128*(CNN_3D_in_channel-1), out_class)
        else:
            self.classifier = nn.Linear(128, out_class)
...
# TODO HRN骨干替换测试版本
class HRN_test(nn.Module):# 测试HRN替换不同的骨干网络
    def __init__(self, dataset):
        super(HRN_test, self).__init__()
        self.dataset = dataset
        self.modes_number = modes_number
        self.num_modalities = len(modes_number)
        self.out_class = out_class
        ...
        from TPPI.models.SSRNSSTNNet import SSNet_AEAE_IN
        from TPPI.models.SSFTTnet import SSFTTnet
        from TPPI.models.SSFTTnet import TransHSI
        self.branches = nn.ModuleList()
        for i, channels in enumerate(self.modes_number):
            Basic_backbone = CNN_3D(dataset=dataset, CNN_3D_in_channel=channels)
            # Basic_backbone = SSFTTnet(dataset = dataset, SSFTTnet_in_channel = channels)
            self.branches.append(Basic_backbone)

    def forward(self, x, weights):

        # 外部权重（Recall）
        weights_tensor = torch.from_numpy(weights).float().to(x.device)
        outs = []
        start = 0
        ...
        for i, channels in enumerate(self.modes_number):
            ...
            start = end
            # 输入到骨干网络
            out = self.branches[i](modality_input)
            outs.append(out)

        # 拼接所有模态输出
        # shape: [B, num_modalities, num_classes]
        outputs = torch.stack(outs, dim=1)

        # 加权融合
        # 扩展权重到 batch 维度
        weights_expanded = weights_tensor.unsqueeze(0)  # [1, M, C]
        # 加权
        if len(outputs.shape) == 2:
            outputs = outputs.transpose(0, 1)
            outputs = outputs.unsqueeze(0)
        outputs_weighted = torch.sum(outputs * weights_expanded, dim=1)
        # 输出
        multi_modal = [
            outputs[:, i, :] * weights_tensor[i].unsqueeze(0)
            for i in range(self.num_modalities)
        ]
