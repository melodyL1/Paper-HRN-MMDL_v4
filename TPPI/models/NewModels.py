...
class MultiModelTrans(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        spectral_x3 = modality_input[self.spectral3]
...
class MultiModelTrans_test_2(nn.Module): # 去掉了VV与VH的3DCNN+Transformer中的Transformer

    def __init__(self, dataset, num_tokens=4, dim=128, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1):
        super(MultiModelTrans_test_2, self).__init__()
        ...
        self.spe_conv3d02 = nn.Sequential(
            nn.Conv3d(in_channels=channelsNum[0], out_channels=channelsNum[1], kernel_size=(5, 3, 3),
                      stride=(1, 1, 1),
                      padding=(2, 1, 1)),
            nn.BatchNorm3d(channelsNum[1]),
            nn.ReLU(inplace=True),
            nn.Dropout3d(p=0.1),
            nn.Conv3d(in_channels=channelsNum[1], out_channels=channelsNum[0], kernel_size=(7, 3, 3),
                      stride=(1, 1, 1),
                      padding=(3, 1, 1)),
            nn.BatchNorm3d(channelsNum[0]),
            nn.ReLU(inplace=True),
        ...
        self.spa_conv2d01 = nn.Sequential(
            nn.Conv2d(in_channels=channelsNum[0] * (
                        self.modes_number[self.spectral1] + self.modes_number[self.spectral2]),
                      out_channels=channelsNum[1], kernel_size=(1, 1), stride=(1, 1),
                      padding=(0, 0)),
            nn.BatchNorm2d(channelsNum[1]),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.1),
        ...
        self.spa_conv2d01b = nn.Sequential(
            nn.Conv2d(in_channels=channelsNum[0] * self.modes_number[self.spectral3], out_channels=channelsNum[1],
        ...
        self.FE = nn.Sequential(
            nn.Conv2d(in_channels=self.modes_number[self.Spatial], out_channels=channelsNum[1], kernel_size=(1, 1),
                      stride=(1, 1)),
            nn.BatchNorm2d(channelsNum[1]),
        )
        self.spa_conv2d02 = nn.Sequential(
        ...
        self.spa_conv2d03 = nn.Sequential(
            nn.Conv2d(in_channels=channelsNum[1] * 3, out_channels=channelsNum[2], kernel_size=(3, 3),
                      stride=(1, 1),
                      padding=(1, 1)),
            nn.BatchNorm2d(channelsNum[2]),
            nn.ReLU(inplace=True),
...
class MultiModelTrans_test_3(nn.Module): # 去掉了时序NDVI的3DCNN+Transformer中的Transformer

    def __init__(self, dataset, num_tokens=4, dim=128, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1):
        super(MultiModelTrans_test_3, self).__init__()
...
class MultiModelTrans_test_4(nn.Module): # 去掉了时序多光谱的2DCNN+Transformer中的Transformer

    def __init__(self, dataset, num_tokens=4, dim=128, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1):
        super(MultiModelTrans_test_4, self).__init__()
...
class MultiModelTrans_test_5(nn.Module): # 去掉了模态融合后的Multi-HeadSelfAttention

    def __init__(self, dataset, num_tokens=4, dim=128, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1):
        super(MultiModelTrans_test_5, self).__init__()
...
class MultiModelTrans_test_6(nn.Module): # 去掉了时序NDVI数据

    def __init__(self, dataset, num_tokens=4, dim=128, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1):
        super(MultiModelTrans_test_6, self).__init__()
        ...
        self.spa_conv2d03 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=channelsNum[2], kernel_size=(3, 3),
                      stride=(1, 1),
                      padding=(1, 1)),
            nn.BatchNorm2d(channelsNum[2]),
            nn.ReLU(inplace=True),
...
class MultiModelTrans_test_7(nn.Module): # 去掉了VV数据

    def __init__(self, dataset, num_tokens=4, dim=128, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1):
        super(MultiModelTrans_test_7, self).__init__()
        ...
        self.spa_conv2d01 = nn.Sequential(
            nn.Conv2d(in_channels=channelsNum[0] * self.modes_number[self.spectral2],
                      out_channels=channelsNum[1], kernel_size=(1, 1), stride=(1, 1),
                      padding=(0, 0)),
            nn.BatchNorm2d(channelsNum[1]),
            nn.ReLU(inplace=True),
...
class MultiModelTrans_test_8(nn.Module): # 去掉了VH数据

    def __init__(self, dataset, num_tokens=4, dim=128, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1):
        super(MultiModelTrans_test_8, self).__init__()
...
class MultiModelTrans_test_9(nn.Module): # 去掉了多光谱数据

    def __init__(self, dataset, num_tokens=4, dim=128, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1):
        super(MultiModelTrans_test_9, self).__init__()
        ...
        self.spa_conv2d03 = nn.Sequential(
            nn.Conv2d(in_channels=channelsNum[1] * 2, out_channels=channelsNum[2], kernel_size=(3, 3),
                      stride=(1, 1),
                      padding=(1, 1)),
            nn.BatchNorm2d(channelsNum[2]),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x, mask=None):
        ...
        x = torch.cat([spectral_x, spectral_xb], dim=1)
        # 通道数 = channelsNum[1]*cat函数拼接个数
        x = self.spa_conv2d03(x)
