...
class SSFTTnet(nn.Module):
    def __init__(self, dataset, in_channels=1, num_classes=NUM_CLASS, num_tokens=4, dim=64, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1, SSFTTnet_in_channel=None):
        super(SSFTTnet, self).__init__()
        if SSFTTnet_in_channel is None:
            SSFTTnet_in_channel = in_channel
        self.dataset = dataset
        self.L = num_tokens
        self.cT = dim
        ...
        self.conv2d_features = nn.Sequential(
            nn.Conv2d(in_channels=8*SSFTTnet_in_channel, out_channels=64, kernel_size=(3, 3)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
...
class TransHSI(nn.Module):
    ...
    def __init__(self, dataset, num_tokens=4, dim=128, depth=1, heads=8, mlp_dim=8, dropout=0.1, emb_dropout=0.1, TransHSI_in_channel=None):
        super(TransHSI, self).__init__()
        if TransHSI_in_channel == None:
            TransHSI_in_channel = in_channel
        with open("configs/config.yml") as fp:
            cfg = yaml.load(fp, Loader=yaml.FullLoader)
        ...
        # in_channels=channelsNum[0]*Band15
        self.spa_conv2d01 = nn.Sequential(
            nn.Conv2d(in_channels=channelsNum[0]*TransHSI_in_channel, out_channels=channelsNum[1], kernel_size=(1, 1), stride=(1, 1),
                      padding=(0, 0)),
            nn.BatchNorm2d(channelsNum[1]),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.1),
        ...
        self.spa_conv2d03 = nn.Sequential(
            nn.Conv2d(in_channels=channelsNum[1]*2+TransHSI_in_channel, out_channels=channelsNum[2], kernel_size=(3, 3), stride=(1, 1),
                      padding=(1, 1)),
            nn.BatchNorm2d(channelsNum[2]),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.1),
