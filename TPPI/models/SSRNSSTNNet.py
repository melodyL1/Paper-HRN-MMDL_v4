...
class SPCModuleIN(nn.Module):
    def __init__(self, in_channels, out_channels, bias=True):
        ...
        if int(in_channel/2) <= 0:
            self.s1 = nn.Conv3d(in_channels, out_channels, kernel_size=(1, 1, 1), stride=(2, 1, 1),
                                bias=False)
        else:
            self.s1 = nn.Conv3d(in_channels, out_channels, kernel_size=(int(in_channel/2), 1, 1), stride=(2, 1, 1), bias=False)
        #self.bn = nn.BatchNorm3d(out_channels)
...
class SPAModuleIN(nn.Module):
    def __init__(self, in_channels, out_channels, k=49, bias=True):
        ...
        if int(in_channel/2)-2 <= 0:
            self.s1 = nn.Conv3d(in_channels, out_channels, kernel_size=(1, 3, 3), bias=False)
        else:
            self.s1 = nn.Conv3d(in_channels, out_channels, kernel_size=(int(in_channel/2)-2,3,3), bias=False)
...
class SSRN(nn.Module):
    def __init__(self, dataset, k=49):
        ...
        self.layer4 = SPAModuleIN(28, 28, 5)
        self.bn4 = nn.BatchNorm2d(28)
        
        self.layer5 = ResSPA(28, 28)
        self.layer6 = ResSPA(28, 28)

        self.fc = nn.Linear(28, out_class)

    def forward(self, x):

        x = self.layer1(x)
        x = F.leaky_relu(x) #self.bn1(F.leaky_relu(self.layer1(x)))
        #print(x.size())
        x = self.layer2(x)
        x = self.layer3(x)
        #x = self.layer31(x)
...
class SSNet_EEAA_IN(nn.Module):
    def __init__(self,dataset, num_classes=16, msize=16, inter_size=49):
...
class SSNet_AEAE_IN(nn.Module):
    def __init__(self, dataset,num_classes=16, msize=16, inter_size=49,SSNet_AEAE_IN_in_channel =None):
        super(SSNet_AEAE_IN, self).__init__()
        if SSNet_AEAE_IN_in_channel is None:
            SSNet_AEAE_IN_in_channel = in_channel
        self.dataset=dataset
#         self.layer1 = SPCModuleIN_(1, 1, inter_size=inter_size) 
#         self.bn1 = nn.BatchNorm2d(inter_size)
        self.layer1 = nn.Sequential(nn.Conv2d(SSNet_AEAE_IN_in_channel, inter_size, 1),
                                   #nn.LeakyReLU(),
                                   nn.BatchNorm2d(inter_size),)
                                   #nn.LeakyReLU())
#         self.layer1 = SPC1d(stride=[2,1], padding=[0,0])
        ...
        self.fc = nn.Linear(msize,out_class)
