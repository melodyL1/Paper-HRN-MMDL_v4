    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, 
        ...
        if self.sr_ratio > 1:
            self.sr = nn.Sequential(
                nn.Conv2d(dim, dim, kernel_size=1, stride=1, groups=dim, bias=True),
...
class CMT(nn.Module):
    def __init__(self, img_size=11, in_chans=in_channel, num_classes=out_class, embed_dims=[46,92,184,368], stem_channel=16, fc_dim=1280,
                 num_heads=[1,2,4,8], mlp_ratios=[3.6,3.6,3.6,3.6], qkv_bias=True, qk_scale=None, representation_size=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=None,
                 depths=[2,2,10,2], qk_ratio=1, sr_ratios=[1,1,1,1], dp=0.1):
        super().__init__()
        ...
        # 下面这段代码img_size的2D卷积核大小，patch_size是2D卷积核步长。
        if PPsize-2 <= 0:
            ...
            self.patch_embed_b = PatchEmbed(
                img_size=1, patch_size=1, in_chans=embed_dims[0], embed_dim=embed_dims[1])
            self.patch_embed_c = PatchEmbed(
                img_size=1, patch_size=1, in_chans=embed_dims[1], embed_dim=embed_dims[2])
            self.patch_embed_d = PatchEmbed(
                img_size=1, patch_size=1, in_chans=embed_dims[2], embed_dim=embed_dims[3])
        else:
            self.patch_embed_a = PatchEmbed(
                img_size=PPsize-2,
                patch_size=1,
                ...
                embed_dim=embed_dims[0])
            self.patch_embed_b = PatchEmbed(
                img_size=PPsize-2, patch_size=1, in_chans=embed_dims[0], embed_dim=embed_dims[1])
            self.patch_embed_c = PatchEmbed(
                img_size=PPsize-2, patch_size=1, in_chans=embed_dims[1], embed_dim=embed_dims[2])
            self.patch_embed_d = PatchEmbed(
                img_size=PPsize-2, patch_size=1, in_chans=embed_dims[2], embed_dim=embed_dims[3])
        ...
        if representation_size:
            ...
            self.pre_logits = nn.Sequential(OrderedDict([
                ('fc', nn.Linear(fc_dim, representation_size)),
                ('act', nn.Tanh())
            ]))
        else:
...
class CMT3DCNN(nn.Module):
    def __init__(self, dataset,img_size=11, in_chans=24, num_classes=out_class, embed_dims=[46, 92, 184, 368], stem_channel=24,
                 fc_dim=1280,
        ...
        self.FE1 = nn.Sequential(
            nn.Conv3d(in_channels=1, out_channels=channelsNum, kernel_size=(int(in_channel/2), 1, 1), stride=(int(in_channel/2), 1, 1)),
            nn.BatchNorm3d(channelsNum),
        )
        self.spe_conv1 = nn.Sequential(
            nn.Conv3d(in_channels=channelsNum, out_channels=channelsNum, kernel_size=(7, 1, 1), stride=(1, 1, 1), padding=(3, 0, 0)),
            nn.BatchNorm3d(channelsNum),
        ...
        self.CF = nn.Sequential(
            nn.Conv3d(in_channels=channelsNum, out_channels=128, kernel_size=(2, 1, 1),
                      stride=(1, 1, 1)),
            nn.BatchNorm3d(128),
        )
        ...
        self.spa_conv1 = nn.Sequential(
            nn.Conv2d(in_channels=channelsNum, out_channels=channelsNum, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
            nn.BatchNorm2d(channelsNum),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.3),
            nn.Conv2d(in_channels=channelsNum, out_channels=channelsNum, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
