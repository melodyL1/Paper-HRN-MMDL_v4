...
from TPPI.models.nextvit_utils import  merge_pre_bn
...
class ConvBNReLU(nn.Module):
    def __init__(
        ...
        self.norm = nn.BatchNorm2d(num_features=out_channels, eps=NORM_EPS)
...
class NextViT(nn.Module):
    def __init__(self, dataset,stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], path_dropout=0.1, attn_drop=0, drop=0, num_classes=1000,
        ...
        self.stem = nn.Sequential(
            ConvBNReLU(in_channel, stem_chs[0], kernel_size=3, stride=2),
        ...
        self.proj_head = nn.Sequential(
            nn.Linear(output_channel, out_class),
        )
    ...
    def forward(self, x):
        ...
        x = self.proj_head(x)
        return x
class SSRNNextViT(nn.Module):
    def __init__(self, dataset,stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], path_dropout=0.1, attn_drop=0, drop=0, num_classes=20,
                 strides=[1, 2, 2, 2], sr_ratios=[8, 4, 2, 1], head_dim=32, mix_block_ratio=0.75,
                 use_checkpoint=False):
        super(SSRNNextViT, self).__init__()
        self.use_checkpoint = use_checkpoint
        self.dataset = dataset
        channelsNum = 24
        if int(in_channel/2) <= 0:
            self.FE1 = nn.Sequential(
                nn.Conv3d(in_channels=1, out_channels=channelsNum, kernel_size=(1, 1, 1), stride=(2, 1, 1)),
                nn.BatchNorm3d(channelsNum),
            )
        else:
            self.FE1 = nn.Sequential(
                nn.Conv3d(in_channels=1, out_channels=channelsNum, kernel_size=(int(in_channel/2), 1, 1), stride=(2, 1, 1)),
                nn.BatchNorm3d(channelsNum),
            )
        ...
        if int(in_channel/2)-2 <= 0:
            self.CF = nn.Sequential(
                nn.Conv3d(in_channels=channelsNum, out_channels=128, kernel_size=(1, 1, 1),
                          stride=(1, 1, 1)),
                nn.BatchNorm3d(128),
            )
        else:
            self.CF = nn.Sequential(
                nn.Conv3d(in_channels=channelsNum, out_channels=128, kernel_size=(int(in_channel/2)-2, 1, 1),
                          stride=(1, 1, 1)),
                nn.BatchNorm3d(128),
            )
        ...
        self.stem = nn.Sequential(
            ConvBNReLU(channelsNum, stem_chs[0], kernel_size=3, stride=2),
            ConvBNReLU(stem_chs[0], stem_chs[1], kernel_size=3, stride=1),
            ConvBNReLU(stem_chs[1], stem_chs[2], kernel_size=3, stride=1),
            ConvBNReLU(stem_chs[2], stem_chs[2], kernel_size=3, stride=2),
        )
