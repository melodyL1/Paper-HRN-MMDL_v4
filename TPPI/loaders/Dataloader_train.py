...
def get_dataLoader(data, cfg,logdir):
    ...
    if cfg["Model"] == 'Shallow_Network':
        if cfg["Model_detail"]["Shallow_Multimodal"] == False:
            ...
            for j, channels in enumerate(modes_number):
                end = start + channels
                x_data_Multimodal = x_data[:, :, :, start:end]
                start = end
                x_train_reshaped = x_data_Multimodal.reshape((x_data_Multimodal.shape[0], -1))
                concatenated_data = np.concatenate((x_train_reshaped, y_data.reshape(-1, 1)), axis=1)
                x_Shallow_Network.append(x_train_reshaped)
                y_Shallow_Network.append(y_data.reshape(-1, 1))
                df = pd.DataFrame(concatenated_data, columns=[f'Feature_{i}' for i in range(x_train_reshaped.shape[1])] + ['Label'])
                df.to_csv(logdir + f"/data{j}.csv", index=False)
    else:
        x_Shallow_Network = 0
        y_Shallow_Network = 0
